...
app.get('/getTopOutput', (req, res) => {
    exec('sudo tegrastats | head -n 1', (error, stdout, stderr) => {
        ...
        const emcFreqMatch = output.match(/EMC_FREQ \d+%@\d+/)[0].replace("%@", "% ")+"MHz";
        const gr3dFreq = output.match(/GR3D_FREQ \d+%@\[\d+/)[0].replace("%@[", "% ")+"MHz";
        const videoEncoding = output.match(/NVENC .*? NVDEC off NVJPG off NVJPG1 off VIC .*? OFA off NVDLA0 off NVDLA1 off PVA0_FREQ off APE \d+/)[0]
            .replace("%@", "% ")
            .replace(/ NVDEC/g, '\nNVDEC')
            .replace(/ NVJPG/g, '\nNVJPG')
            .replace(/ NVJPG1/g, '\nNVJPG1')
            ...
            .replace(/ PVA0_FREQ/g, '\nPVA0_FREQ')
            .replace(/ APE/g, '\nAPE') + "MHz";
        const temperatures = output.match(/cpu@.*?C tboard@.*?C soc2@.*?C tdiode@.*?C soc0@.*?C tj@.*?C soc1@.*?C/)[0]
            .replace("@", " ")
            .replace("@", " ")
            .replace("@", " ")
            .replace("@", " ")
